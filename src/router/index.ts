...
if (
      ...
      (to.path === "/login" ||
        to.path === "/register" ||
        to.path === "/admin/login" ||
        to.path === "/") // 👉 THÊM DÒNG NÀY ĐỂ BẮT TRANG CHỦ
    ) {
      ...
      switch (role) {
        ...
        default:
          // Nếu là Khách hàng (hoặc user không có role Admin/Staff)
          // 1. Nếu đang cố vào trang chủ ("/") -> Cho phép vào
          if (to.path === "/") {
             return next();
          }
          // 2. Nếu đang cố vào trang login/register -> Đẩy về trang chủ
          return next("/");
      }
    }
