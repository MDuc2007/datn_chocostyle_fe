        ...
        <div class="main-product-area">
          ...
          <div class="toolbar">
            <div class="search-wrapper">
              ...
              <input 
                ...
                @input="applyFilters" 
                placeholder="Tìm kiếm theo tên áo khoác..." 
                class="search-input"
              />
          ...
          <div v-else>
            ...
            <div v-else class="empty-state">
              ...
              <p>Rất tiếc, chúng tôi không tìm thấy áo khoác nào phù hợp với tìm kiếm hoặc bộ lọc của bạn.</p>
...
// ĐÃ SỬA: THÊM DEBOUNCE ĐỂ TRÁNH SPAM API KHI GÕ TÌM KIẾM
...
const applyFilters = () => {
  ...
  searchTimeout = setTimeout(() => {
    currentPage.value = 0;
    fetchFilteredData();
  }, 500); // Đợi 0.5s sau khi ngừng gõ mới lọc
}

// Lọc ngay lập tức khi đổi Select Box (Sắp xếp)
...
const fetchFilteredData = async (isAppend = false) => {
  ...
  try {
    ...
    // Vẫn truyền keyword xuống BE (để BE tìm theo mã SP nếu cần)
    ...
    // Chuẩn bị text tìm kiếm cho Frontend
    const keywordLower = searchKeyword.value.trim().toLowerCase();

    rawData.forEach(item => {
      let passFilter = true;

      // 👉 ĐÃ THÊM: LỌC THEO TÊN SẢN PHẨM Ở FRONTEND
      if (keywordLower) {
...
/* Banner Full Width giống ảnh 2 (Màu Nền Đỏ đậm) */
.sub-banner {
  width: 100%;
...
/* Breadcrumb thiết kế chuẩn ảnh 1 */
...
.loading-state, .error-state, .empty-state { text-align: center; padding: 60px 20px; border-radius: 12px; background-color: #fafafa; border: 1px dashed #e5e7eb; margin-top: 20px; }
.spinner { border: 3px solid #f3f3f3; border-top: 3px solid #63391F; border-radius: 50%; width: 36px; height: 36px; animation: spin 1s linear infinite; margin: 0 auto 16px; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }

.error-icon svg { width: 48px; height: 48px; color: #d32f2f; margin-bottom: 15px; }
.error-state p { color: #666; margin-bottom: 20px; font-size: 15px; }

/* CSS MỚI CHO EMPTY STATE */
.empty-icon-wrapper { margin-bottom: 20px; display: inline-block; padding: 20px; background-color: #f3f4f6; border-radius: 50%; }
.empty-icon-wrapper svg { width: 64px; height: 64px; color: #9ca3af; }
.empty-state h4 { margin: 0 0 10px 0; font-size: 20px; font-weight: 600; color: #374151; }
.empty-state p { color: #6b7280; font-size: 15px; margin-bottom: 25px; max-width: 400px; margin-left: auto; margin-right: auto; line-height: 1.5; }
.btn-clear-filter { background: #63391F; color: white; padding: 12px 28px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.3s ease; box-shadow: 0 4px 6px rgba(99, 57, 31, 0.2); }
.btn-clear-filter:hover { background: #4a2a17; transform: translateY(-2px); box-shadow: 0 6px 12px rgba(99, 57, 31, 0.3); }
