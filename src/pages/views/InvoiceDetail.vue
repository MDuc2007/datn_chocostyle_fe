          ...
          <div v-else>
            ...
            <div class="dashboard-info-grid no-print">
              <div class="detail-card">
                <div class="card-header-clean" style="justify-content: space-between; display: flex;">
              ...
              <div class="detail-card">
                <div class="card-header-clean">
                  <div class="header-icon">
                    <img src="/src/assets/icon/shopping-cart-svgrepo-com.svg" style="width: 24px; height: 24px"
                      alt="" />
                  </div>
                    ...
                    <tr v-for="(p, index) in invoice.sanPhamList" :key="index">
                      ...
                      <td class="light-text" style="vertical-align: middle;">
                        <div style="display: flex; flex-direction: column; align-items: center;">
                          ...
                          <div v-if="getPriceChangeHistory(p.idSpct)" class="price-warning-badge">
                      ...
                      <td class="quantity-text">
                        <div v-if="canEditProducts" class="qty-control-wrapper">
                          ...
                          <button class="btn-qty" @click="updateProductQuantity(p, p.soLuong + 1)">+</button>
                        </div>
      ...
      <div class="modal-card modal-lg-custom">
        ...
        <div class="form-row-custom">
          <div class="modal-input-wrapper">
            ...
            <input type="text" v-model="customerForm.ten" class="modern-input" />
          </div>
          <div class="modal-input-wrapper">
            <label class="input-label">Số điện thoại</label>
            <input type="text" v-model="customerForm.sdt" class="modern-input" />
          </div>
        </div>
        ...
        <div class="form-row-custom">
          <div class="modal-input-wrapper">
            ...
            <select v-model="selectedProvince" @change="handleProvinceChange" class="modern-input">
          ...
          <div class="modal-input-wrapper">
            ...
            <select v-model="selectedDistrict" @change="handleDistrictChange" :disabled="!selectedProvince"
              class="modern-input">
          ...
          <div class="modal-input-wrapper">
            ...
            <select v-model="selectedWard" @change="handleWardChange" :disabled="!selectedDistrict"
              class="modern-input">
        ...
        <div class="modal-input-wrapper">
          ...
          <textarea v-model="customerForm.diaChiCuThe" class="modern-textarea" rows="2"
            placeholder="Ví dụ: Số 10, Ngõ 12..."></textarea>
        </div>

        <div class="modal-footer-modern mt-3">
          <button class="btn-modern btn-secondary" @click="showCustomerModal = false">Hủy</button>
          <button class="btn-modern btn-primary-modern" @click="saveCustomerInfo">Lưu thông tin</button>
        </div>
      </div>
    </div>
...
const getAuthHeaders = () => {
  ...
  const token = userStr ? JSON.parse(userStr).accessToken : localStorage.getItem("token");
...
const canEditOrder = computed(() => {
  ...
  const isCash = invoice.value.loaiDon === 1 ||
...
const customerForm = reactive({ ten: '', sdt: '', diaChiCuThe: '', thanhPho: '', quan: '', phuong: '' });
const showCustomerModal = ref(false);

const getProvinces = async () => {
  try {
    const res = await axios.get(`${GHN_API_BASE}/master-data/province`, { headers: { token: GHN_TOKEN } });
...
const getDistricts = async (provinceId: any) => {
  try {
    const res = await axios.get(`${GHN_API_BASE}/master-data/district`, {
      headers: { token: GHN_TOKEN }, params: { province_id: provinceId },
    });
...
const getWards = async (districtId: any) => {
  try {
    const res = await axios.get(`${GHN_API_BASE}/master-data/ward`, {
      headers: { token: GHN_TOKEN }, params: { district_id: districtId },
    });
...
const mapAddressFromText = async (cityName: string, districtName: string, wardName: string) => {
  try {
    if (provinces.value.length === 0) await getProvinces();
    const foundProvince = provinces.value.find(p => p.ProvinceName.toLowerCase().trim() === cityName.toLowerCase().trim() || p.NameExtension?.some((ext: string) => ext.toLowerCase() === cityName.toLowerCase().trim()));
    ...
    if (foundProvince) {
      ...
      const foundDistrict = districts.value.find(d => d.DistrictName.toLowerCase().trim() === districtName.toLowerCase().trim() || d.NameExtension?.some((ext: string) => ext.toLowerCase() === districtName.toLowerCase().trim()));
      if (foundDistrict) {
        ...
        const foundWard = wards.value.find(w => w.WardName.toLowerCase().trim() === wardName.toLowerCase().trim() || w.NameExtension?.some((ext: string) => ext.toLowerCase() === wardName.toLowerCase().trim()));
        if (foundWard) {
...
const openEditCustomerModal = async () => {
  ...
  if (invoice.value.diaChi) {
    ...
    if (parts.length >= 4) {
      ...
      await mapAddressFromText(customerForm.thanhPho, customerForm.quan, customerForm.phuong);
    } else {
...
const saveCustomerInfo = async () => {
  if (!customerForm.ten || !customerForm.sdt || !customerForm.diaChiCuThe || !customerForm.phuong) {
  ...
  try {
    await axios.put(`http://localhost:8080/api/hoa-don/${invoice.value?.id}/thong-tin-nhan-hang`, null, {
      params: { ten: customerForm.ten, sdt: customerForm.sdt, diaChi: fullAddress },
    ...
    alert(e.response?.data || "Lỗi khi cập nhật thông tin!");
  }
};
...
const updateProductQuantity = async (item: InvoiceProduct, newQty: number) => {
  if (newQty < 1) return;
  // NOTE: Bắt buộc ở hàm getDetail trong file HoaDonService.java (Spring Boot)
  // bạn phải map .idSpct(ct.getIdSpct().getId()) vào HoaDonSanPhamResponse thì item.idSpct mới có giá trị nhé!
  try {
    ...
    alert(e.response?.data || "Lỗi khi cập nhật số lượng!");
  }
};
...
const getPriceChangeHistory = (idSpct: number) => {
  ...
  const log = invoice.value.lichSuList.find(l =>
    l.hanhDong === 'Thay đổi giá sản phẩm' && l.ghiChu.includes(`[PRICE_CHANGE] ID ${idSpct}`)
  );
  if (log) {
...
// ===============================================
// CÁC LOGIC CHUNG (Mặc định giữ nguyên)
// ===============================================
...
// FIX TÌNH TRẠNG HIỂN THỊ TRÙNG BƯỚC TIMELINE
  ...
  for (const log of sortedHistory) {
    ...
    if (!seenStatuses.has(log.trangThai)) {
      ...
      uniqueSteps.push({
        ...
        icon: log.trangThai === 5 ? "/src/assets/icon/cancel-svgrepo-com.svg" : "/src/assets/icon/check-svgrepo-com.svg",
        time: log.thoiGian,
      });
    }
  }
...
<style scoped>
/* ================== CSS CHO SELECT ĐỊA CHỈ TRONG MODAL ================== */
.modal-lg-custom {
  max-width: 600px;
}
