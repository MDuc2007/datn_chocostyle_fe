...
async function toggleStatus(item) {
  ...
  try {
    await axios.put(
      `http://localhost:8080/api/chat-lieu/${item.id}/doi-trang-thai`,
      null,
      {
...
const closeModal = () => {
  ...
  newColor.value = { tenChatLieu: "", nguoiTao: "admin" };
  isEdit.value = false;
...
const editColor = (item) => {
  ...
  newColor.value.tenChatLieu = item.name;
  openModal();
};
...
const deleteColor = async (item) => {
  if (!confirm(`Bạn có chắc muốn xóa chất liệu "${item.name}"?`)) return;

  try {
    await axios.delete(`http://localhost:8080/api/chat-lieu/${item.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    showNotification("Xóa chất liệu thành công", "success");
    fetchColors();
  } catch {
    showNotification("Xóa chất liệu thất bại", "error");
  }
};
</script>
