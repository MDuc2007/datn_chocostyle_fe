...
const addColor = async () => {
  if (!newColor.value.tenLoai.trim()) return;

  await fetch("http://localhost:8080/api/loai-ao", {
    method: "POST",
...
const updateColor = async () => {
  ...
  await fetch(`http://localhost:8080/api/loai-ao/${editingId.value}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      tenLoai: newColor.value.tenLoai,
...
const deleteColor = async (item) => {
  if (confirm(`Bạn có chắc muốn xóa loại áo "${item.name}"?`)) {
    await fetch(`http://localhost:8080/api/loai-ao/${item.id}`, {
      method: "DELETE",
    });
    fetchColors();
  }
