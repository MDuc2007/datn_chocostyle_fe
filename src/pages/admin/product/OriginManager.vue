<template>
  <div class="header">
    <h2 class="title">QUẢN LÝ XUẤT XỨ</h2>
    ...
    <div class="top-bar" style="margin-top: 30px">
      ...
      <div class="add-btn">
        <button @click="openModal"><span>＋</span> Thêm xuất xứ</button>
      </div>
    </div>
  </div>
  ...
  <div v-if="isModalOpen" class="modal-overlay" @click="closeModal">
    <div class="modal" @click.stop>
      <h3>{{ isEdit ? "Sửa xuất xứ" : "Thêm xuất xứ" }}</h3>

      <input v-model="newColor.tenXuatXu" placeholder="Nhập tên" />
...
const editColor = (item) => {
  ...
  newColor.value.tenXuatXu = item.name;
  openModal();
};
...
const deleteColor = async (item) => {
  if (confirm(`Bạn có chắc muốn xóa xuất xứ "${item.name}"?`)) {
    await fetch(`http://localhost:8080/api/xuat-xu/${item.id}`, {
      method: "DELETE",
    });
    fetchColors();
  }
