...
async function handleModalConfirm() {
  if (modal.value.action === "TOGGLE_STATUS") {
    ...
    try {
      ...
      item.trangThai = oldStatus; // rollback
...
const validateKieuDang = (name, id = null) => {
  if (!name || !name.trim()) {
    return "Tên kiểu dáng không được để trống";
  }

  const length = name.trim().length;

  if (length < 2) {
    return "Tên kiểu dáng phải từ 2 ký tự trở lên";
  }

  if (length > 50) {
    return "Tên kiểu dáng không được quá 50 ký tự";
  }

  if (!/^[a-zA-ZÀ-ỹ\s]+$/.test(name)) {
    return "Tên kiểu dáng không được chứa số hoặc ký tự đặc biệt";
  }
...
const addColor = async () => {
  const error = validateKieuDang(newColor.value.tenKieuDang);
...
const updateColor = async () => {
  const error = validateKieuDang(newColor.value.tenKieuDang, editingId.value);
