...
const addColor = async () => {
  if (!newColor.value.tenKichCo.trim()) return;

  await fetch("http://localhost:8080/api/kich-co", {
    method: "POST",
...
const updateColor = async () => {
  ...
  await fetch(`http://localhost:8080/api/kich-co/${editingId.value}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      tenKichCo: newColor.value.tenKichCo,
...
const deleteColor = async (item) => {
  if (confirm(`Bạn có chắc muốn xóa kích cỡ "${item.name}"?`)) {
    await fetch(`http://localhost:8080/api/kich-co/${item.id}`, {
      method: "DELETE",
    });
    fetchColors();
  }
