          ...
          <div v-if="activeTab === 'info'" class="tab-content fade-in">
            ...
            <div class="form-grid-container">
              ...
              <div class="form-group">
                ...
                <input type="date" :value="userInfo.dob" class="form-input" readonly />
              </div>
            </div>
          </div>
...
onMounted(async () => {
  ...
  if (userStr && token) {
    try {
      ...
      // Xử lý thông minh: Quét tìm các từ khóa liên quan đến Admin trong toàn bộ dữ liệu
  ...
  try {
    ...
    const res = await axios.post(`http://localhost:8080/api/nhan-vien/${userInfo.value.id}/avatar`, formData, {
      headers: {
        ...
        // TUYỆT ĐỐI KHÔNG THÊM 'Content-Type': 'multipart/form-data' VÀO ĐÂY ĐỂ TRÁNH LỖI 401
      }
    });
...
.form-input { padding: 10px 15px; height: 45px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none; background: #fff; transition: all 0.2s; box-sizing: border-box; width: 100%; }
