  ...
  stompClient.connect({}, () => {
    ...
    stompClient.subscribe("/topic/shift-updates", (message: any) => {
      if (message.body === "UPDATED") {
        console.log(
          "🔄 Nhận tín hiệu có đơn hàng mới -> Đang tải lại danh sách ca...",
        );
          ...
          <div class="modal-header-custom">
            <div>
              <h3 style="margin: 0; font-size: 18px; color: #fff">
                SAO KÊ CHI TIẾT CA LÀM VIỆC
              </h3>
              <p style="margin: 4px 0 0 0; font-size: 13px; opacity: 0.9;color: white;">
                Hệ thống đối soát dòng tiền
              </p>
            </div>
          ...
          <div class="modal-body-custom">
              ...
              >
                <div style="display: flex; gap: 8px; align-items: center">
                  <span style="color: #64748b; font-size: 14px"
                    >NV được phân công:</span
                  >
                  <strong style="font-size: 14px; color: #334155">{{
                    selectedShift.nhanVien
                  }}</strong>
                </div>
                <div style="display: flex; gap: 8px; align-items: center">
                  <span style="color: #64748b; font-size: 14px"
                ...
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  >
                    <template v-if="selectedShift.nguoiMoCa">
                      <span
                        v-if="formatStaffName(selectedShift.nguoiMoCa).code"
                        class="staff-code-badge"
                      >
              ...
              >
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  <strong
                    ...
                    :class="{
                      'text-red-600': selectedShift.trangThai !== 1,
                      'color: #334155': selectedShift.trangThai === 1,
                    }"
                  >
                    {{
                      selectedShift.trangThai === 1
                        ? selectedShift.thoiGianDong
                        : "Chưa đóng ca"
                    }}
                  </strong>
                </div>
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  >
                    <template v-if="selectedShift.nguoiDongCa">
                      <span
                        v-if="formatStaffName(selectedShift.nguoiDongCa).code"
                        class="staff-code-badge"
                      >
            ...
            <div class="stats-section">
              <h4 class="section-title">
                📊 Thống kê phát sinh (Doanh thu tăng trong ca)
              </h4>
              <div class="stats-cards">
                ...
                <div class="stat-card">
                  ...
                  <strong class="text-blue-600"
                    >+
                    {{ formatCurrency(selectedShift.doanhThuTienMat) }}</strong
                  >
                </div>
                <div class="stat-card">
                  <span>Chuyển khoản thu thêm</span>
                  <strong class="text-blue-600"
                    >+ {{ formatCurrency(selectedShift.doanhThuCk) }}</strong
                  >
                </div>
              </div>
            </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="bg-gray-50 font-bold">
                      {{
                        formatCurrency(
                          selectedShift.tienMatDauCa +
                            selectedShift.doanhThuTienMat,
                        )
                      }}
                    </td>
                    <td class="font-bold text-blue-600">
                      {{ formatCurrency(selectedShift.tienMat) }}
                    </td>
                    <td :class="getDiffClass(selectedShift.chenhLechTienMat)">
                      {{ selectedShift.chenhLechTienMat > 0 ? "+" : ""
                      }}{{ formatCurrency(selectedShift.chenhLechTienMat) }}
                    </td>
                  </tr>
                  <tr>
                    <td>💳 Chuyển khoản</td>
                    ...
                    <td class="bg-gray-50 font-bold">
                      {{
                        formatCurrency(
                          selectedShift.tienChuyenKhoanDauCa +
                            selectedShift.doanhThuCk,
                        )
                      }}
                    </td>
                    ...
                    <td :class="getDiffClass(selectedShift.chenhLechCk)">
                      {{ selectedShift.chenhLechCk > 0 ? "+" : ""
                      }}{{ formatCurrency(selectedShift.chenhLechCk) }}
                    </td>
                  </tr>
                </tbody>
              </table>
...
.custom-table {
  ...
  border-spacing: 0;
  min-width: 1100px;
} /* Mở rộng min-width để chứa đủ cột */
.custom-table th {
  font-weight: 700;
  padding: 16px 12px;
  text-align: left;
