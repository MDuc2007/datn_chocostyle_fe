...
const getSchedulesForShiftAndDate = (idCa: number, dateStr: string) => {
  ...
  return schedules.value.filter((s) => {
    ...
    const matchEmpName =
      ...
      s.nhanVien?.hoTen?.toLowerCase().includes(searchName) ||
      s.nhanVien?.maNv?.toLowerCase().includes(searchName);
    const matchStatus =
...
watch(
  [() => filters.employeeName, () => filters.date, () => filters.status],
  () => {
...
const confirmUpdateSeries = async () => {
  ...
  try {
    ...
    if (updateOption.value === "one") {
      ...
      showToast("Đã cập nhật chuỗi", "success");
    }
    showUpdateOptionModal.value = false;
    originalForm.value = null;
    closeModal();
    refreshData();
  } catch (error: any) {
...
const confirmDeleteSeries = async () => {
  ...
  try {
    if (deleteOption.value === "one") {
      ...
      showToast("Đã xóa chuỗi", "success");
    }
    refreshData();
  } catch (error: any) {
    showToast(error.response?.data?.message || "Lỗi", "error");
...
const openAddModal = () => {
  isEditing.value = false;
  form.id = null;
  form.ngayLamViec = filters.date || getTodayStr();
  form.idNhanVien = "";
  form.idCa = "";
  form.ghiChu = "";
  form.trangThai = 2;
...
const closeModal = async () => {
  if (isEditing.value && hasFormChanged.value) {
    if (!(await showConfirmDialog("Hủy thay đổi?"))) return;
  }
...
const openDetailPopup = (event: MouseEvent, item: Schedule) => {
...
const calendarDays = computed(() => {
  ...
  if (calendarView.value === "month") {
    ...
    days.push({
      day: curr.getDate(),
      dateStr: `${curr.getFullYear()}-${String(curr.getMonth() + 1).padStart(2, "0")}-${String(curr.getDate()).padStart(2, "0")}`,
    });
  }
  return days;
});
...
const getEventStyle = (
  ...
  if (overlapping.length > 0) {
    const sorted = [...overlapping, sche].sort(
      (a, b) =>
        timeToMinutes(a.caLamViec.gioBatDau) -
        timeToMinutes(b.caLamViec.gioBatDau),
    );
  ...
  return {
    ...
    left: `${leftOffset}%`,
    width: overlapping.length > 0 ? `${widthOffset}%` : "auto",
    right: overlapping.length > 0 ? "auto" : "2px",
    zIndex: 10,
  };
};
...
<template>
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in pagedSchedules" :key="item.id">
              ...
              <td class="text-center">
                ...
                <div
                  class="tooltip-wrapper tooltip-right"
                  data-tooltip="Xóa"
    ...
    >
      <div class="calendar-header">
        ...
        <div class="legend-mini">
          <div class="legend-item">
            <span class="dot active"></span> Đang mở
          </div>
          <div class="legend-item">
            <span class="dot working"></span> Đang làm
          </div>
        ...
        >
          ...
          <div v-if="cell.day" class="shifts-container">
            <div
              ...
              class="shift-item"
              :class="{
                draft: sche.trangThai === 1,
                working: sche.trangThai === 3,
              }"
      ...
      <div v-else-if="calendarView === 'week'" class="week-matrix-view">
        <div class="matrix-header">
          ...
          >
            <div class="day-name">
              {{
                weekDays.find((d) => d.val === new Date(cell.dateStr).getDay())
                  ?.label
              }}
            </div>
          ...
          >
            ...
            >
              <button
                class="cell-add-btn"
                @click.stop="
                  openAddModalWithDateAndShift(cell.dateStr, shift.idCa)
                "
              ...
              <div
                v-for="sche in getSchedulesForShiftAndDate(
                  shift.idCa,
                  cell.dateStr,
                )"
                :key="sche.id"
      ...
      <div v-else class="week-matrix-view">
        <div class="matrix-header" style="grid-template-columns: 200px 1fr">
          <div
            class="matrix-cell-head shift-col-head"
          ...
          <div class="matrix-cell-head today" style="border-right: none">
            <div class="day-name">
              {{
                weekDays.find(
                  (d) => d.val === new Date(calendarDays[0]?.dateStr).getDay(),
                )?.label || "Ngày"
              }}
            </div>
            <div class="day-date">
              {{ formatDate(calendarDays[0]?.dateStr) }}
            </div>
          </div>
        </div>
        ...
        <div class="matrix-body">
          ...
          >
            <div
              class="matrix-cell shift-info-cell"
              style="
              ...
              "
            >
              <div class="shift-name" style="font-size: 15px">
                {{ shift.tenCa }}
              </div>
  ...
  <div
    v-if="showDetailPopup && selectedSchedule"
    class="modal-backdrop z-highest"
      ...
      <div class="detail-body">
        ...
        <div class="detail-row info">
          ...
          <div class="content">
            ...
            <div class="sub-text">
              {{ formatTime(selectedSchedule.caLamViec?.gioBatDau) }} -
              {{ formatTime(selectedSchedule.caLamViec?.gioKetThuc) }}
            </div>
          </div>
        </div>
        ...
        <div class="detail-row info">
          ...
          <div class="content">
            ...
            <div class="sub-text">
              Mã: {{ selectedSchedule.nhanVien?.maNv }}
            </div>
          </div>
        </div>
        ...
        <div class="detail-row info">
          ...
          <div class="content">
            <span
              class="status-tag"
              :class="{
                active: selectedSchedule.trangThai === 2,
                draft: selectedSchedule.trangThai === 1,
                working: selectedSchedule.trangThai === 3,
              }"
            >
              {{
                selectedSchedule.trangThai === 2
                  ? "Đang mở"
                  : selectedSchedule.trangThai === 3
                    ? "Đang làm"
                    : "Đã đóng"
              }}
            </span>
...
.day-head-cell:last-child {
  border-right: none;
}

.day-head-cell .day-name {
  font-size: 11px;
  font-weight: 700;
  color: #6b7280;
  text-transform: uppercase;
...
.day-head-cell.today .day-name {
  color: #63391f;
}
.day-head-cell.today .day-num {
  background: #63391f;
  color: white;
}
...
.day-column:hover {
  background-color: rgba(99, 57, 31, 0.02);
}
...
.matrix-cell-head.today {
  background: #fff8f5 !important;
}
.matrix-cell-head.today .day-name,
.matrix-cell-head.today .day-date {
