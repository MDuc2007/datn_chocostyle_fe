      ...
      <transition name="fade" mode="out-in">
        ...
        >
          ...
          <div class="retry-link">
            Chưa nhận được mã? 
            <span v-if="countdown > 0" class="disabled-text">
              Gửi lại sau {{ countdown }}s
            </span>
            <span v-else @click="handleSendOtp" class="active-text">
              Gửi lại
            </span>
          </div>
        </form>
...
const handleSendOtp = async () => {
  ...
  if (!email.value || email.value.trim() === "") {
    isError.value = true;
    message.value = "Vui lòng nhập email liên kết!";
    return;
  }
  ...
  if (!emailRegex.test(email.value)) {
    isError.value = true;
    message.value = "Định dạng email không hợp lệ!";
    return;
  }
  ...
  try {
    ...
    if (err.response?.data?.message) {
      ...
      message.value = "Không tìm thấy tài khoản với email này!";
    }
  } finally {
    loading.value = false;
  }
